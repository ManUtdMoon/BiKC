...
def main(checkpoint, num_episodes):
    torch.backends.cudnn.benchmark = False
    torch.backends.cudnn.deterministic = True

    proj_dir = pathlib.Path(__file__).parent.parent.parent.expanduser()
    ckpt_path = proj_dir / checkpoint
    output_dir = ckpt_path.parent.parent / "demo"
    pathlib.Path(output_dir).mkdir(parents=True, exist_ok=True)

    # load model
    k_payload = torch.load(open(ckpt_path, 'rb'), pickle_module=dill)
    k_cfg = k_payload['cfg']
    ...
    # load dataset
    dataset_dir = proj_dir / "data/aloha/datasets" / k_cfg.task.dataset.task
    ...
    with tqdm(total=num_episodes, desc="Process", mininterval=1.0) as pbar:
        ...
        for i in range(num_episodes):
            ...
            with h5py.File(dataset_path, "r+") as demo:
                ...
                last_keypose = qpos.copy()
                obs_shape_meta = k_cfg.shape_meta.obs
                for i, img in enumerate(imgs):
                    # form input, compute next keypose
                    obs_dict = dict()
                    if "qpos" in obs_shape_meta.keys():
                        obs_dict["qpos"] = torch.tensor(
                            qpos[i:i+1], dtype=torch.float32
                        ).to(device)
                    if "last_keypose" in obs_shape_meta.keys():
                        obs_dict["last_keypose"] = torch.tensor(
                            last_keypose[i:i+1], dtype=torch.float32
                        ).to(device)
                    obs_dict["images"] = torch.tensor(
                        img.transpose(2, 0, 1)[None, ...] / 255.0,
                        dtype=torch.float32
                    ).to(device)

                    next_keypose = k_policy.predict_next_keypose(obs_dict).to("cpu").numpy().squeeze()

                    # TODO: how to specify last keypose for given demo
                    # if i < len(qpos) - 1:
                    #     last_keypose[i+1] = next_keypose
