...
class ReplayBufferWithKeypose:
    ...
    def __init__(self, 
        ...
        assert('keyposes' in root['meta'])
        for key, value in root['data'].items():
            assert(
                value.shape[0]
                == root['meta']['episode_ends'][-1]
                == root['meta']['keyposes'][-1] + 1
            )
        self.root = root
    ...
    def create_empty_zarr(cls, storage=None, root=None):
        ...
        if 'keyposes' not in meta:
            keyposes = meta.zeros('keyposes', shape=(0,), dtype=np.int64,
                compressor=None, overwrite=False)
    ...
    def create_empty_numpy(cls):
        root = {
            ...
            'meta': {
                ...
                'keyposes': np.zeros((0,), dtype=np.int64)
            }
        }
        return cls(root=root)
    ...
    @property
    def keyposes(self):
        return self.meta['keyposes']
    
    @property
    def subtraj_ends(self):
        keypose_indices = self.keyposes[:].astype(np.int64)
        return keypose_indices + 1
    ...
    def add_episode(self, 
        ...
        ### keyposes update
        # append to keyposes
        keyposes = self.keyposes
        assert new_keyposes[0] == 0
        new_keyposes = new_keyposes[1:] # remove the 0th step
        if is_zarr:
            keyposes.resize(keyposes.shape[0] + len(new_keyposes))
        else:
            keyposes.resize(keyposes.shape[0] + len(new_keyposes), refcheck=False)
        keyposes[-new_keyposes.shape[0]:] = new_keyposes + curr_len

        # rechunk
        if is_zarr:
            if keyposes.chunks[0] < keyposes.shape[0]:
                rechunk_recompress_array(self.meta, 'keyposes', 
                    chunk_length=int(keyposes.shape[0] * 1.5))
    ...
    def drop_episode(self):
        ...
        keyposes = self.keyposes[:].copy()
        keyposes_except_last_epi = keyposes[keyposes < start_idx]
        if is_zarr:
            self.keyposes.resize(len(keyposes_except_last_epi))
        else:
            self.keyposes.resize(len(keyposes_except_last_epi), refcheck=False)

    
    def pop_episode(self):
        assert(self.n_episodes > 0)
        episode, local_keyposes = self.get_episode(self.n_episodes-1, copy=True)
        self.drop_episode()
        return episode, local_keyposes
    ...
    def get_episode(self, idx, copy=False):
        ...
        result, local_keyposes = self.get_steps_slice(start_idx, end_idx, copy=copy)
        return result, local_keyposes
    
    def get_episode_slice(self, idx):
        raise NotImplementedError("Invalid for keyposes, do not use")
        start_idx = 0
        if idx > 0:
            start_idx = self.episode_ends[idx-1]
        end_idx = self.episode_ends[idx]
    ...
    def get_steps_slice(self, start, stop, step=None, copy=False):
        ...
        keyposes = set(self.keyposes)
        s = set(range(start, stop, step if step is not None else 1))
        keyposes = keyposes.intersection(s)
        global_keyposes = np.array(sorted(list(keyposes)), dtype=np.int64)
        local_keyposes = global_keyposes - start

        return data, local_keyposes
